...
export default function FloatingAICoach({ userProfile }: FloatingAICoachProps) {
  ...
  return (
    <>
      ...
      {/* Chat Panel */}
      {isOpen && !isMinimized && (
        <Card className="fixed bottom-6 right-6 z-50 w-96 h-[500px] shadow-2xl border-2 border-primary/20 bg-card">
      ...
      {/* Minimized State */}
      {isOpen && isMinimized && (
        <Card className="fixed bottom-6 right-6 z-50 w-64 shadow-lg border-primary/20 bg-card cursor-pointer" onClick={() => setIsMinimized(false)}>
